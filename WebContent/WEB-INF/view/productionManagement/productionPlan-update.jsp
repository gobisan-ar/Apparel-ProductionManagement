...
<body>
						...
						<div class="row">
							...
							<div class="col-md-12 mt-lg-4 mt-4">
								<div
									...
									<h1 class="h3 mb-0 text-gray-800">Update Production Plan: ${production.productionID}</h1>
								</div>
							</div>
							...
							<div class="col-md-12 mt-3 mb-3">
								<div class="card">
									<div class="card-body px-4">
										<form class="row g-4 px-4 mt-2" action="planUpdate" id="planForm" name="planForm">
										
										<input type="text" id="productionID" name="productionID" value="${production.productionID}" hidden/>
										<input type="text" id="noticeID" name ="noticeID" value="${production.noticeID}" hidden>
										<input type="text" id="routeID" name ="routeID" value="${route.routeID}" hidden>	
										<input type="text" id="scheduleID" name ="scheduleID" value="${schedule.sheduleID}" hidden>
											...
											<div class="row mt-4">
												<div class="col-md-6">
													...
													<select class="form-select mt-1" id="line" name="line" aria-label="Default select example">
														<option value="${route.lineID}" selected>Line ${route.lineID}</option>
														<c:forEach var="line" items="${lineList}">
															<c:if test="${line.lineID ne route.lineID}">
												...
												<div class="col-md-6">
													...
													<select class="form-select mt-1" id="supervisor" name="supervisor" aria-label="Default select example">
														<c:forEach var="sup" items="${supervisorlist}">
															<c:if test="${supervisor eq sup.supID}">
																<option value="${sup.supID}" selected>${sup.supName}</option>
															</c:if>
															<c:if test="${supervisor ne sup.supID}">
											...
											<div class = "row mt-4">
												<div class="col-md-6">
													...
													<input type="Date" class="form-control" id="startDate" name="startDate" value="${schedule.startDate}">
												</div>
	
												<div class="col-md-6">
													<label for="endDate" class="form-label">End Date:</label> 
													<input type="Date" class="form-control" id="endDate" name="endDate" value="${schedule.endDate}">
												</div>
											</div>
											...
											<div class="row mt-4">
												...
												<div class="col-3 mb-4">
													<button type="reset" class="btn btn-secondary mx-4">
	...
	<script>
		...
		$(".packagingDays").hide();
				...
				<c:forEach var="unit" items="${route.units}">
				<c:if test="${unit.key eq 'cutting' and unit.value eq 1}">
					$("#cuttingUnit").prop('checked', true);
					$(".cuttingDays").show();
				</c:if>
				
				<c:if test="${unit.key eq 'printing' and unit.value eq 1}">
					$("#printingUnit").prop('checked', true);
					$(".printingDays").show();
				</c:if>
				
				<c:if test="${unit.key eq 'sewing' and unit.value eq 1}">
					$("#sewingUnit").prop('checked', true);
					$(".sewingDays").show();
				</c:if>
				
				<c:if test="${unit.key eq 'washing' and unit.value eq 1}">
					$("#washingUnit").prop('checked', true);
					$(".washingDays").show();
				</c:if>
				
				<c:if test="${unit.key eq 'pressing' and unit.value eq 1}">
					$("#pressingUnit").prop('checked', true);
					$(".pressingDays").show();
				</c:if>
				
				<c:if test="${unit.key eq 'packaging' and unit.value eq 1}">
					$("#packagingUnit").prop('checked', true);
					$(".packagingDays").show();
				</c:if>
			</c:forEach>
			
			<c:forEach var="unitDays" items="${schedule.unitDays}">
			<c:if test="${unitDays.key eq 'cutting'}">
				$("#cuttingDays").val(${unitDays.value});
			</c:if>
			
			<c:if test="${unitDays.key eq 'printing'}">
				$("#printingDays").val(${unitDays.value});
			</c:if>
			
			<c:if test="${unitDays.key eq 'sewing'}">
				$("#sewingDays").val(${unitDays.value});
			</c:if>
			
			<c:if test="${unitDays.key eq 'washing'}">
				$("#washingDays").val(${unitDays.value});
			</c:if>
			
			<c:if test="${unitDays.key eq 'pressing'}">
				$("#pressingDays").val(${unitDays.value});
			</c:if>
			
			<c:if test="${unitDays.key eq 'packaging'}">
				$("#packagingDays").val(${unitDays.value});
			</c:if>
		</c:forEach>
